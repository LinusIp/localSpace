...
try {
  ...
  if (gotSvg.status !== 200 || gotSvg.type !== "image/svg+xml" || !gotSvg.disposition.startsWith("attachment;") || !svgText.startsWith("<svg xmlns=") || !["from", "the", "second", "window"].every((w) => svgText.includes(w))) {
    throw new Error(`the SVG download is wrong: ${JSON.stringify({ ...gotSvg, bytes: gotSvg.bytes.length, head: svgText.slice(0, 80) })}`);
  }
