...
| 5 whiteboard as the first package | **done** on 2026-09-11: logic, tools, context provider, evals, Automerge document; `@localspace/canvas` with snapping, `@localspace/ui`, the web surface with a replica in the frame, a sync state per replica, undo as a forward change, the gate walk as a browser test, package layouts from a script, a CI workflow | three measurements open: the 60 fps number and the evals on the reference model, one W32 trip; the workflow's first run | | the W32 trip and a remote |
| 6 retrieval, upload, gateway, web tools | gateway modes, `web.search` and `web.fetch`; **6.0 done on 2026-09-12**: PNG and SVG export as `image.v1` and `svg.v1` artifacts, `io.localspace.types` as the first types package with kinds checked at install, file documents of their own with a download | BM25 for `find_capability` only | tantivy, usearch, ACL pre-filter, upload | 2 weeks |
