...
> - **Landed on 19 September 2026:** a download continues from where it stopped, after a lost connection and after closing the app, and a finished file is the model's only when its SHA-256 is the published one. **Models can be copied in instead of downloaded:** a file with a catalog entry's name and its SHA-256, placed in the models folder (`%LOCALAPPDATA%\localSpace\models` on Windows), counts as that model and nothing is fetched; a file that is something else does not count and is left alone. The default recommendation only offers models whose licence permits commercial use, and every model shows its licence in plain words. The app starts the model that was in use last by itself.
> - **Landed on 18 September 2026:** the Windows installer and a portable zip, with the inference engine inside (llama.cpp's Vulkan release, pinned by SHA-256). They are built by the `package` workflow and are **not signed yet**: SmartScreen warns, and Smart App Control, where it is on, refuses them outright. Of what §A2 and §A3 say, the pasted Hugging Face repo id is not in the build (cut for the laptop test on 19 September).
