...
| H§4.1 | Surfaces | **built** | `widgets` and `web` views render in the web client, a `web` view in a sandboxed frame on the harness's own origin; `egui` views render only in the egui client, whose reference surface was retired on 2026-09-11. `stream` is declared and refused with a clear message. The whiteboard's web surface carries real direct manipulation on `@localspace/canvas`: tool palette, marquee and shift multi-select, dragging a selection, eight resize handles, snapping with guides, stacking order, locking, duplicate, freehand ink, text labels and keyboard shortcuts, with one commit per gesture rather than one per frame. |
...
| H§5.2 | Three surface kinds | **partial** | `widgets` and `web` built in the web client; `egui` in the egui client only; `stream` needs Tier B frame transport. |
| H§5.3 | The `egui` surface ABI | **partial** | `hs_alloc` / `hs_init` / `hs_frame`, postcard, one `i64` return, run by the egui client's `SurfaceRunner` under wasmtime. Its reference surface and the conformance test against it were retired on 2026-09-11; the ABI goes with the egui client (`docs/DECISIONS.md`). |
| H§5.4 | Surface ↔ logic | **built** | The document carries state and messages carry commands, as specified. A `web` surface holds the document as an Automerge replica with its own sync state in Core; a surface that takes JSON reads the projection and follows `DocChanged`. |
...
| H§16.3 | Client and transport | **partial** | The Client repaints only on input or when Core wakes it through the transport — never on a timer. An `egui` surface runs `hs_frame` only when input arrived, a document or message is waiting, or it asked for a repaint; a quiet host frame repaints last frame's cached meshes without entering the guest, and a document the guest already holds is not pushed at it again (both were covered by the egui client's surface test, retired with its reference surface on 2026-09-11). **No zstd on the wire, no brotli bundle, no hardware video encode** (there is no `stream` surface yet). |
